...
reciever='aa@ants.ftp.sh'; 
recpath='~/mo/'; 
command="rsync -a --info=name1 "; [ "$2" ]&& command="$2"; hello=hello; [ $3 ]&& hello=$3; 
...
printf " $c2 Command:"; read -rep " " -i "$command" "command";
$command --no-motd $(for i in "${CHECKED[@]}"; do realpath $i -z; printf " "; done; ) ${reciever}:${recpath}; 
printf "\nGG\n\n sent: \n"; 
