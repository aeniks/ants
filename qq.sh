...
cc1="$(tput hpa 26; cut -c1-44 $option|sed -n 1p 2>/dev/null|head -n1 -c $((COLUMNS-28))
tput hpa 26; cut -c1-44 $option|sed -n 2p 2>/dev/null|pr --omit-header --page-width=$((COLUMNS-28)))"; fi;

local index="$1" option="$2"
if value_in_array "$index" "${selected_options[@]}"; then
content+="\n$color  $SELECTED $option$re$dim${cc1//"\n"/}$re\n\n"
else
content+="\n$color  $UNSELECTED $option$re$dim${cc1//"\n"/}$re\n\n"
fi
