...
do echo -e "\e[0m  $c2 Installing $i \e[2m"; sleep 1; if [ -z "$2" ]; then bash $i; else $2 $i; fi; cd $OLDPWD; 
...
echo -e "hello from $(date)" >> "README.md";
...
###########
## timer - countdown
12_klockan () { while true; do tput sc; tput cup 0 $((COLUMNS - 42)); tput setaf $(echo $LINENO) 2>/dev/null; date +%F%t$(tput setaf $(date +%u))%A$(tput setaf $(date +%S))%t%R:%S; tput rc; sleep 1; done& } 
