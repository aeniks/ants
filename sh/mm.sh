...
## MENU FUNCTION ####
echo -ne "$dim ---------------------------
 ------$re BASH-SELECTOR$dim ------
 --------------------------- $re\n
 Select the desired option using their number $re("$dim"again to uncheck,$re ENTER$dim when done"$re"): \n\n\e[s";
MENU() {
cole=$(stty size|tr " " "\n"|tail -n1)
for nm in ${!opt[@]}; do echo -e " ["$green"${choices[nm]:- }"$re"]" $dim$((nm+1))$re") \
$(echo ${opt[nm]}|head -c40)\e[40G$dim$(sed -n 2p ${opt[nm]}\
|tr -d '\\'|pr -t -c1 -w$((cole-52)))$re"; done; echo "$err";
}
## MENU LOOP ########
...
err="$(echo -ne "\n$dim Invalid option: $re$sel\e[0m";)"; fi; 
#if [ $sel == q ]; then echo kk;
done;
##
