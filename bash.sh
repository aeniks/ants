...
tty="$(tty)"; tty="${tty:(-1):1}"
[ "$PREFIX" ]&& model="$(getprop ro.product.model ro.product.model ro.build.version.min_supported_target_sdk ro.build.version.sdk ro.product.abilist ro.product.name ro.soc.manufacturer ro.soc.model gsm.sim.operator.alpha 2>/dev/null )"; 
...
# [ -e "/usr/bin/gcalcli" ]&& 
# model="$(getprop ro.product.system.model; ls)";  
# printf "$re${dim}··········$re\n"; 
# date="$(date)"; 
