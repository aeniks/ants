#!/bin/bash
# menu for bash 
unset OPTIONS_VALUES OPTIONS_LABELS SELECTED OPTIONS_STRING active defaults options option retval selected idx *;
...
OPTIONS_VALUES=($(ls --color=no $pwd/))
OPTIONS_LABELS=($(cat $pwd/*|tr " " "_"|pr --columns=1 --width=42 --omit-header|head ))
##OPTIONS_LABELS=($(cat ${OPTIONS_VALUES[@]}))
...
do OPTIONS_STRING+="${OPTIONS_VALUES[$i]} (${OPTIONS_LABELS[$i]});"; done;
...
##
for i in "${CHECKED[@],,}";
do echo -e "\t Installing -- $i --;" doots 2>/dev/null; tput cuu 2 ed ; 
##
doots() {
for i in {1..8}; do tput setaf $((RANDOM%256)); echo -ne "."; sleep .2; done& disown;
}
...
for i in ${OPTION_VALUES[@]}; do OPTIONS_LABELS+=("$(cat $i|head -c2)"); done 
