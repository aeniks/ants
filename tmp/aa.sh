...
cc1="$(tput hpa 18; sed -n 1p $option|head -c $((COLUMNS-28)))"
cc2="$(tput hpa 18 cud 1; sed -n 2p $option|head -c $((COLUMNS-28)))"
fi 
local index="$1" option="$2"
if value_in_array "$index" "${selected_options[@]}"; then
content+="\n$color  $SELECTED $option$re$dim$cc1$cc2$re\n\n"
else
content+="\n$color  $UNSELECTED $option$re$dim$cc1$cc2$re\n\n"
fi
