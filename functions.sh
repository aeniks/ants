...
menu() { size=($(stty size)); for i in $(seq $size); do echo; done; 
...
if [[ $1 ]]; then OPTIONS_VALUES=($1); else OPTIONS_VALUES=($(ls -p $PWD/$1)); fi; 
...
OPTIONS_STRING+="$dots\e[6G ${OPTIONS_VALUES[$i]} \e[22G ${OPTIONS_LABELS[$i]};"; done;
...
if [[ $key = "" ]]; then if [[ ${active} == $((idx - 1)) ]]; then echo -e "c"; else echo enter; fi; fi; 
...
echo -e "\e[0m $c2 $i$green Installed$re \e[2m"; done; echo -e "\n Done"; fi; echo -e "\e[0m"; 
}; ## END MENU ##
#################
## 12_ menu #####
#################
