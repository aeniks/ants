...
tmpfile="$PREFIX/tmp/sanity${PWD////_}"
echo -e "\e[31m"; find ./ -maxdepth $md -type f|grep "[ {}()'']"|cat|tee ${tmpfile}| \
while read ff; do 
echo -e "$ff\e[32m >> "$(echo -e "\e[36m$ff"|sed "s/ /_/g"|sed "s/[{}()\'~,]//g")""; 
echo -ne "\e[31m"; done; 
read -n1 -rep "$c2 Are these changes ok? ${red}n ${re}/${green} y " "baba"; fi; 
if [ $baba = "y" ]; 
then cat ${tmpfile}|while read fa; do mv -nv "$fa" "$(echo $fa |sed "s/ /_/g"| \
sed "s/[{}()\'~,]//g")"; done; fi; echo -e "ok \e[0m"; ls -p; }
